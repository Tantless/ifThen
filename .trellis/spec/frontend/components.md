...
## Analysis Progress Status UI

Use this pattern when displaying long-running analysis jobs.

### Scope / Trigger

- Trigger: UI displays `JobRead` analysis progress from the backend.
- The always-visible chat/list area should communicate current activity, not imply whole-job completion.
- The detail dialog may show multiple `running` stages at the same time.

### Contract

Frontend should consume `JobRead.stages` through `resolveJobProgress()` / `resolveJobStages()` and render:

```typescript
type FrontAnalysisStage = {
  id: string;
  label: string;
...
- Always-visible area: text status + click target only; do not render a global horizontal progress bar.
- Detail dialog: list every stage; render a progress bar for each `running` stage.
- Multiple `running` stages are valid and expected after summaries complete.
- Use a real `<button>` for the clickable progress entry when it is not nested inside another button.
- Do not parse `status_message`; it is backend diagnostic text only.
...
```tsx
<button type="button" onClick={onOpenDetails} aria-label="查看分析进度详情">
...
```tsx
// Wrong: this recreates the misleading global progress bar problem.
<div className="front-progress__fill" style={{ width: `${progress.percent}%` }} />
```

### Tests Required

- Adapter test for multiple running stages.
- Static shell test that compact progress has a details entry and no always-visible progress fill.
- UI interaction test when behavior changes: click compact entry -> dialog opens -> running stages show their own progress bars.
...
```typescript
const label = job.status_message?.includes("snapshot") ? "关系快照" : "分析中";
```

#### Correct

```typescript
const runningStages = resolveJobStages(job).filter((stage) => stage.status === "running");
```
