...
当前剩余工作：
- 真实性路线图的实现型阶段已经完成到 `realism-08`。
- 后续只剩验收层收口：provider/人工拟真性验收、未来泄漏人工复核、完整分析 pipeline 性能复测。
...
5. `realism-02`：实现分层证据上下文和实时会话 memory pack。（已完成）
6. `realism-03`：实现检索排序与上下文预算。（已完成）
7. `realism-04`：增强人格与表达风格。（已完成）
8. `realism-05`：补齐实时回复 prompt 与泄漏护栏。（已完成）
9. `realism-08`：沉淀质量验收与回归策略。（已完成，剩余 provider/人工/性能验收）

`realism-01-pre` 完成条件：
- [x] 生成 3 段每段 1000+ 条消息的合成聊天记录。
- [x] 每段 `conversation.txt` 均可被现有导入解析器解析。
- [x] 每段包含关键可改写点、cutoff 后真相揭示和评估标注。
- [x] 样例能证明“只截断历史会误判，而利用后续客观事实作为 modeler-only evidence 能更真实”。
- [x] 不提交 API key，不使用真实姓名、真实学校、真实联系方式等可识别信息。

`realism-01` 完成条件：
- [x] 至少建立 10 个固定拟真性评估样例。
