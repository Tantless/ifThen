...
本报告基于一次历史外部 5000 条性能样本的真实 worker 运行结果。当前真实性路线图的测试集已经更正为仓库内合成语料 `D:\ifThen\tests\fixtures\realism_synthetic`；后续 realism 验收和性能抽样应以该目录为准。运行时使用隔离数据目录，没有修改应用正常使用的 `.data` 数据库。

## 运行配置

| 项目 | 值 |
| --- | --- |
| 数据集 | 历史外部 5000 条性能样本；当前 realism 测试集为 `D:\ifThen\tests\fixtures\realism_synthetic` |
| 文件大小 | 446,350 bytes |
| 文件声明消息数 | 5,000 |
| 实际解析消息数 | 5,000 |
