...
`main` 当前已经跑通了“导入聊天记录 -> 异步分析 -> 历史浏览 -> 改写消息 -> 反事实推演 -> 桌面端展示”的完整闭环，桌面主界面与聊天记录弹窗也已完成一轮可用性打磨；当前已经补齐可分发的 Windows zip release，剩余的最高优先级工作聚焦在项目结构重构。
...
### 2026-04-12：Windows 发布链路落地

- 已新增 `npm run release:win` 一键发布命令
- Electron 发布态会直接携带 PyInstaller 构建的 `api / worker` 可执行文件
- 发布态默认数据目录切到用户目录，日志随数据目录落盘
- 已产出 `desktop/release/If Then-0.1.0-x64.zip`，可在 Windows 解压后直接运行
...
- 已具备可分发的 Windows zip release；安装器与自动更新仍未开始
