...
目标：先证明当前哪里不真实，并建立“未来证据可用于建模、不可用于角色台词”的硬合同。
...
1. PR1：建立评估样例和当前失败分类。
2. PR2：扩展 context pack，加入分层证据结构。
3. PR3：修改 prompt，加入未来证据使用和泄漏护栏。
4. PR4：实现检索排序和上下文预算。
5. PR5：加入 persona/style 低成本统计。
6. PR6：新增实时分支会话后端模型和接口。
7. PR7：新增实时分支会话前端体验。
8. PR8：补齐质量验收、回归脚本和 rollout 文档。
